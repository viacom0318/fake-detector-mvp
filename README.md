import streamlit as st
import pandas as pd
import networkx as nx
import matplotlib.pyplot as plt
from datetime import datetime, timezone
from difflib import SequenceMatcher

# 文字相似度函數
def text_similarity(a, b):
    return SequenceMatcher(None, a.lower(), b.lower()).ratio()

# Mock數據（測試用，模擬真實API回傳）
def get_mock_user_data(username):
    mock_data = {
        "elonmusk": {  # 正常帳號
            "username": "elonmusk",
            "followers": 200000000,
            "following": 500,
            "tweet_count": 30000,
            "created_at": datetime(2006, 6, 1, tzinfo=timezone.utc),
            "recent_tweets": ["Hello world", "Mars mission update", "Tesla news today"] * 10
        },
        "fakebot123": {  # 高風險假帳號
            "username": "fakebot123",
            "followers": 50,
            "following": 2000,
            "tweet_count": 5000,
            "created_at": datetime(2025, 12, 1, tzinfo=timezone.utc),
            "recent_tweets": ["投資賺大錢！私訊我"] * 50
        },
        "normaluser": {  # 中等正常
            "username": "normaluser",
            "followers": 1000,
            "following": 800,
            "tweet_count": 2000,
            "created_at": datetime(2015, 1, 1, tzinfo=timezone.utc),
            "recent_tweets": ["今天吃了好吃的", "工作好累", "看電影推薦"] * 20
        }
    }
    return mock_data.get(username.lower(), None)

def get_mock_retweeters():
    return ["bot1", "bot2", "bot3", "scamaccount", "fakeuser"] * 10  # 模擬50個轉發者

# 主頁面
st.title("🇹🇼 台灣假帳號／假消息檢測 MVP（測試版）")
st.markdown("### 先用mock數據玩玩看，確認介面與邏輯 → 之後再接真實X API")

tab1, tab2 = st.tabs(["假帳號檢測", "傳播溯源"])

with tab1:
    username = st.text_input("輸入測試帳號（試試：elonmusk / fakebot123 / normaluser）", placeholder="elonmusk")
    if st.button("開始檢測"):
        with st.spinner("模擬檢測中..."):
            mock_user = get_mock_user_data(username)
            if not mock_user:
                st.error("無此mock帳號，請試試內建的三個範例")
                st.stop()
            
            # 基本特徵計算
            followers = mock_user["followers"]
            following = mock_user["following"]
            follower_ratio = followers / max(1, following)
            account_age_days = (datetime.now(timezone.utc) - mock_user["created_at"]).days
            tweets_per_day = mock_user["tweet_count"] / max(1, account_age_days)
            
            # 貼文重複度
            tweet_texts = mock_user["recent_tweets"]
            avg_similarity = 0
            if len(tweet_texts) > 5:
                sims = [text_similarity(tweet_texts[i], tweet_texts[j])
                        for i in range(len(tweet_texts))
                        for j in range(i + 1, len(tweet_texts))]
                avg_similarity = sum(sims) / len(sims) if sims else 0
            
            # 規則評分
            fake_score = 0
            if follower_ratio < 0.2:   fake_score += 30
            if tweets_per_day > 20:    fake_score += 30
            if account_age_days < 90:  fake_score += 20
            if avg_similarity > 0.7:   fake_score += 20
            
            # 顯示結果
            st.subheader(f"@{mock_user['username']} 檢測結果")
            col1, col2 = st.columns(2)
            with col1:
                st.metric("追隨者數", followers)
                st.metric("追隨中數", following)
                st.metric("追隨比", f"{follower_ratio:.2f}")
            with col2:
                st.metric("帳號年齡", f"{account_age_days} 天")
                st.metric("每日貼文數", f"{tweets_per_day:.1f}")
                st.metric("貼文平均相似度", f"{avg_similarity:.2f}")
            
            st.progress(fake_score / 100)
            st.write(f"### 假帳號風險評分：**{fake_score}%**（規則估計）")
            if fake_score > 70:
                st.error("🚨 高風險！極可能為假帳號或自動化帳號")
            elif fake_score > 40:
                st.warning("⚠️ 中風險，建議進一步觀察")
            else:
                st.success("✅ 低風險，看起來正常")

with tab2:
    if st.button("模擬溯源（點我看範例網路圖）"):
        with st.spinner("模擬抓取轉發者..."):
            retweeters = get_mock_retweeters()
            st.write(f"找到 **{len(retweeters)}** 位轉發者（模擬數據）")
            st.write(", ".join([f"@{u}" for u in retweeters[:30]]) + " ...")
            
            # 網路圖
            G = nx.Graph()
            G.add_node("原貼文")
            for u in retweeters:
                G.add_node(f"@{u}")
                G.add_edge("原貼文", f"@{u}")
            
            fig, ax = plt.subplots(figsize=(12, 8))
            nx.draw(G, with_labels=True, node_color='lightcoral' if len(retweeters)>20 else 'lightblue',
                    edge_color='gray', node_size=1000, font_size=9)
            st.pyplot(fig)

st.info("👍 這是純mock測試版，跑起來後告訴我感覺如何？想改規則、加功能、美化介面、還是接真實API，都直接說，我馬上幫你調整！")
